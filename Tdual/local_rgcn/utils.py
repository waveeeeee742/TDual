...
def stat_ranks(rank_list, method):
    ...
    print("({}) MRR, Hits@ (1,3,10):{:.6f}, {:.6f}, {:.6f}, {:.6f}".format(
        method, mrr.item(), hit_list[0], hit_list[1], hit_list[2]))
    return mrr, hit_list
...
def split_by_time(data):
    ...
    print("# Sanity Check:  ave node num : {:04f}, ave rel num : {:04f}, snapshots num: {:04d}, "
          "max edges num: {:04d}, min edges num: {:04d}, max union rate: {:.4f}, min union rate: {:.4f}"
...
def load_data(dataset, bfs_level=3, relabel=False):
    ...
    elif dataset in ['ICEWS18', 'ICEWS14', "GDELT", "SMALL", "ICEWS14s", "ICEWS05-15", "YAGO", "WIKI"]:
...
def r2e_super(triplets, num_rels):
    """处理超关系图的r2e"""
    src, rel, dst = triplets.transpose()
...
def build_super_g(num_rels, rel_head, rel_tail, use_cuda, segnn, gpu):
    ...
    if segnn:
        ...
        rel_node_id = torch.arange(0, num_rels*2, dtype=torch.long).view(-1, 1)
