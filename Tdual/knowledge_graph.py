...
class RGCNEntityDataset(object):
    ...
    def load(self, load_time=True):
        ...
        # 打印加载信息
        print(f"\nLoading data files from: {self.dir}")
        print(f"Load time information: {load_time}")

        # 验证文件存在
        for path, name in [(entity_path, 'entity2id.txt'),
        ...
        try:
            ...
            print(f"{'=' * 60}")
            print(f"# Entities: {self.num_nodes}")
            print(f"# Relations: {self.num_rels}")
            print(f"# Train edges: {len(self.train)}")
            print(f"# Valid edges: {len(self.valid)}")
            print(f"# Test edges: {len(self.test)}")
            if load_time and len(self.train) > 0:
                print(f"# Time steps in train: {len(np.unique(self.train[:, 3]))}")
            print(f"{'=' * 60}\n")

        except Exception as e:
            print(f"\n❌ Error loading dataset files:")
...
def load_from_local(dir, dataset):
    ...
    -----------
    dir : str
        数据集根目录，例如 '/autodl-tmp'
    dataset : str
        数据集名称，例如 'ICEWS05-15'

    Returns:
    --------
    data : RGCNLinkDataset
        加载的数据集对象
    """
    import os
    ...
    # 打印调试信息
    print(f"\n{'=' * 60}")
    print(f"Loading dataset from local directory")
    print(f"{'=' * 60}")
    ...
    if missing_files:
        raise FileNotFoundError(
            f"\n❌ Missing required files in {dataset_path}:\n" +
    ...
    # 根据数据集类型决定是否加载时间信息
    # ICEWS系列数据集通常包含时间戳
    load_time = True if any(x in dataset for x in ['ICEWS', 'GDELT', 'YAGO']) else False

    data.load(load_time=load_time)
